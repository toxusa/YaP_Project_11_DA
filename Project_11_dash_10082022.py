                ...
                [
                    ...
                    html.Div(
                        [
                            ...
                            dcc.RangeSlider(
                                min = time.mktime(datetime.strptime(str(dash_visits['dt'].min()), '%Y-%m-%d %H:%M:%S').timetuple()),
                                max = time.mktime(datetime.strptime(str(dash_visits['dt'].max()), '%Y-%m-%d %H:%M:%S').timetuple()),
                                marks = {
                                    1569338880: {'label': pd.to_datetime(dash_visits['dt'].min(), format='%Y-%m-%d %H:%M:%S').strftime('%d.%m %H:%M')},
                                    1569340800: {'label': pd.to_datetime(dash_visits['dt'].max(), format='%Y-%m-%d %H:%M:%S').strftime('%d.%m %H:%M')},
...
def update_figures(time_selector, item_topics, ages):
    ...
    start_dt = datetime.fromtimestamp(time_selector[0]).strftime('%Y-%m-%d %H:%M:%S')
    end_dt = datetime.fromtimestamp(time_selector[1]).strftime('%Y-%m-%d %H:%M:%S')
